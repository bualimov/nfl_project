...
plt.title('Running Average Defensive Player Entropy (Pre-snap Analysis)', 
...
def init():
    ...
    # Add legend with position grouping
    handles = []
    labels_text = []
    for position in sorted(position_colors.keys()):
        position_lines = [line for pid, line in lines.items() if entropy_data[pid]['position'] == position]
        if position_lines:
            handles.extend(position_lines)
            labels_text.extend([pid for pid, line in lines.items() if entropy_data[pid]['position'] == position])
    
    ax.legend(handles, labels_text, bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
...
def update(frame_idx):
    ...
    for player_id, data in entropy_data.items():
        ...
        running_avg = np.mean(current_entropy)
        
        # Create arrays for running average line
        avg_frames = current_frames
        avg_entropy = [running_avg] * len(current_frames)
        
        # Update line
        lines[player_id].set_data(avg_frames, avg_entropy)
        
        # Update marker
        if current_frames:
            markers[player_id].set_data([current_frames[-1]], [running_avg])
            
            # Update label with running average
            labels[player_id].set_position((current_frames[-1], running_avg))
...
# Save animation
anim.save('entropy_avg_graph.gif', writer='pillow', fps=10)
print("\nAnimation saved as 'entropy_avg_graph.gif'") 